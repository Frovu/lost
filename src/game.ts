...
type NodeBase = Position & { cost: number };
export function* neighbors<T extends NodeBase>(grid: T[][], node: T, opts: PathfinderParams) {
	const r = 1;
	const { size } = opts;
	const { x, y } = node;
	for (let i = Math.max(0, x - r); i < Math.min(size, x + r); ++i) {
		for (let j = Math.max(0, x - r); j < Math.min(size, x + r); ++j) {
			if (i === x && j === y)
				continue;
			const cur = grid[i][j];
			if (cur.cost >= 255)
...
export function computeCost(a: NodeBase, b: NodeBase) {
	// FIXME: if r != 1
	return (a.x === b.x || a.y === b.y) ? 1 : SQRT_2;
}
